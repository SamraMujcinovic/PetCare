    ...
    handleValidation(){
     ...
     if(!this.state.email){
      ...
      if(!this.state.password){
        ...
        console.log(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/.test(this.props.password))
        if(this.props.availableEmail) {
          formIsValid = false;
          errors["password"] = "Password not valid (at least 6 characters, 1 big letter, 1 small letter, 1 sign)!";
        }
      }
