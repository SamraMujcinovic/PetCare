...
export default function RasePets({id}) {
    const [rasePets, setRasePets] = React.useState([]);
    ...
    useEffect(() => {
        axios.get(
            `http://localhost:8088/pet_category_service_api/pets/inRase?id=${id}`
        )
        .then((response) => {
            setRasePets(response.data);
        })
      }, [])

    const filteredProducts = rasePets && rasePets.filter(pet => {
        if(
    ...
    const sortRase = (rasePets, index) => {
        if(index < 0 || index > 2) return;
        switch(index) {
           case 0:
                return rasePets.sort((a,b) => {
